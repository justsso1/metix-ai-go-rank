...
test('share helpers post the ranking to a public URL, not localhost', () => {
  ...
  assert.equal(pageUrl, 'https://go.metix.ai/recruiters-view/share/maya-chen-se');

  const text = shareText(result);
  assert.equal(text, "Recruiters find me before 1,193 other people. Didn't know that was a stat until today. See where you stand → go.metix.ai/recruiters-view");
...
test('social copy fills all three templates with the current group data', () => {
  ...
  assert.equal(shareText(atRank(47, 12431)), "Recruiters find me before 12,384 other people. Didn't know that was a stat until today. See where you stand → go.metix.ai/recruiters-view");
  assert.equal(shareText(atRank(143, 1240)), "Apparently I'm ahead of 88% of backend engineers in Seattle in recruiter search. See where you stand → go.metix.ai/recruiters-view");
  assert.equal(shareText(atRank(9883, 12431)), "12,431 backend engineers in Seattle. Recruiters see the first 50. That's the whole game. See where you stand → go.metix.ai/recruiters-view");
});

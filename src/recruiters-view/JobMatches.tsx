...
export default function JobMatches({ result, email, saveEmail }: { result: RankLookupFound; email: string; saveEmail: (email: string) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    trackCampaign("email_request", { source: "job_shortlist", status: "submit" });
    ...
    try {
      ...
      setRequest(saved); trackCampaign("email_request", { source: "job_shortlist", status: "saved" }); saveEmail(saved.email);
    } catch (err) {
      if ((err as Error).name !== 'AbortError') {
        setError((err as Error).message || 'We couldn’t save your request. Please try again.');
        trackCampaign("email_request", { source: "job_shortlist", status: "error" });
      }
    } finally {
  ...
  return <article className="rv-card rv-jobs-card">
    <div className="rv-job-list">
      {matchedJobs(result).slice(0, 3).map((job, index) => job.url ? <a key={job.id} className="rv-job" onClick={() => trackCampaign("job_click", { destination: "external" })} href={job.url} target="_blank" rel="noopener noreferrer" aria-label={`Apply: ${job.title} at ${job.company}`}>
      ...
      </a> : <button key={job.id} type="button" className="rv-job" onClick={() => { trackCampaign("job_click", { destination: "details" }); setSelected(job); }} aria-label={`View details: ${job.title} at ${job.company}`}>
        <span className="rv-job-number">{String(index + 1).padStart(2, '0')}</span>
    ...
    <div className="rv-email-capture rv-shortlist-form" aria-busy={submitting}>
      ...
      {!loaded ? <p role="status">Loading your request…</p> : request ? <p className="rv-success" role="status"><Check size={18} /><span>We’ll email up to 10 job matches from your first ranking to <strong>{request.email}</strong>. Nothing is sent when there are no matches.</span></p> : <form className="rv-email-form" onSubmit={submit} noValidate>
        ...
        <button className="btn btn-primary" type="submit" disabled={submitting} data-track="email_submit_click" data-track-location="opportunities" data-track-source="job_shortlist">{submitting ? 'Submitting…' : 'Send me the shortlist'}</button>
      </form>}

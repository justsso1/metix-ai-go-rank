import { ROUTES, SITE_ORIGIN, shareTaskPath } from './site.ts';
...
export function taskIdFromSharePath(path: string): string | null {
...
export function campaignPage(path: string, search = ''): CampaignPage {
  ...
  if (taskIdFromSharePath(path) !== null || /\/(?:result|share)\/?$/.test(path) || params.has('u') || params.has('task_id') || params.has('taskId')) return 'result';
  return 'entry';
}
